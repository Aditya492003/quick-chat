        ...
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
            <div>
                <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                    <a href="#"><Image className={expand ? "w-36" : "w-10"} src={expand ? assets.logo : assets.deepthink_icon} alt=''  /></a>
                    ...
                    <div onClick={() => expand ? setExpand(false) : setExpand(true)}
                        ...
                        <div className={`absolute w-max ${expand ? "left-1/2 -translate-x-1/2 top-12" : "-top-12 left-0"} opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
                            {expand ? 'Close sidebar' : 'Open sidebarr'}
                ...
                <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-[#212327] hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                    ...
                    {expand && <p className='text-black text font-medium  bg-blue-200 px-10
                     py-2 rounded-full'>New Chat</p>}
                </button>
            ...
            <div>
                <div className={`flex cursor-pointer items-center group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                ...
                <div onClick={user ? null : openSignIn}
                    ...
                    {
                        user ? <UserButton />
                            : <Image src={assets.profile_icon} alt='' className='w7' />
                    }
